/*
 *
 * FocalTech ft5402 TouchScreen driver.
 *
...
#include <linux/input/ft5402_ts.h>
...
struct ft5402_ts_data {
	struct i2c_client *client;
	struct input_dev *input_dev;
	const struct ft5402_ts_platform_data *pdata;
...
//static bool VDD_OPEN =false;

extern int ft5402FL_Init_IC_Param(struct i2c_client * client);


#define LANGSHANG 2
#define FULE 1

static int ft5402_i2c_read(struct i2c_client *client, char *writebuf,
...
static int ft5402_i2c_write(struct i2c_client *client, char *writebuf,
			    int writelen)
{
	int ret;
...
{
	return ft5402_i2c_read(client, &addr, 1, val, 1);
}

static void ft5402_update_fw_ver(struct ft5402_ts_data *data)
{
	...
	err = ft5402_i2c_read(client, &reg_addr, 1, &data->fw_ver[0], 1);
	if (err < 0)
		dev_err(&client->dev, "fw major version read failed");

	reg_addr = FT_REG_FW_MIN_VER;
	err = ft5402_i2c_read(client, &reg_addr, 1, &data->fw_ver[1], 1);
	if (err < 0)
		dev_err(&client->dev, "fw minor version read failed");

	reg_addr = FT_REG_FW_SUB_MIN_VER;
	err = ft5402_i2c_read(client, &reg_addr, 1, &data->fw_ver[2], 1);
	if (err < 0)
...
static struct input_dev *ip_dev;
static irqreturn_t ft5402_ts_interrupt(int irq, void *dev_id)
{
	struct ft5402_ts_data *data = dev_id;
	
	int rc, i;
	u32 id, x, y, status, num_touches;
	u8 reg = 0x00, *buf;
	bool update_input = false;
	static u16 pointstateC = 0;
	static u16 pointstateL = 0;
	if (!data) {
	...
	rc = ft5402_i2c_read(data->client, &reg, 1,
	...
	for(i=0;i<10;i++)
	{
		if((pointstateL&(1<<i))&&((pointstateC&(1<<i))==0))
		{
		//printk("pointstateL = %d, pointstateC=%d,i=%d\n",pointstateL,pointstateC,i);
			input_mt_slot(ip_dev, i);
			input_mt_report_slot_state(ip_dev, MT_TOOL_FINGER, 0);
		}
	}
	pointstateL = pointstateC;
	//printk("\n");
	/*if (update_input) {
...
static int ft5402_power_on(struct ft5402_ts_data *data)
{
	int ret;
	...
	printk("%s\n",__func__);
	if (!IS_ERR(data->vdd)) {
		ret = regulator_set_voltage(data->vdd, FT_VTG_MIN_UV,
					   FT_VTG_MAX_UV);
		if (ret) {
		...
		ret = reg_set_optimum_mode_check(data->vdd,
			10000);
		if (ret < 0) {
			dev_err(&data->client->dev,
	...
	if (!IS_ERR(data->vcc_i2c)) {
		ret = regulator_set_voltage(data->vcc_i2c, FT_I2C_VTG_MIN_UV,
					   FT_I2C_VTG_MAX_UV);
		if (ret) {
			dev_err(&data->client->dev,
		...
		ret = reg_set_optimum_mode_check(data->vcc_i2c,
			10000);
		if (ret < 0) {
			dev_err(&data->client->dev,
...
/**
 * ft5402_power_off - Turn device power OFF
 * @ts: driver private data
 *
 * Returns zero on success, else an error.
 */
static int ft5402_power_off(struct ft5402_ts_data *data)
{
	int ret;
	...
	if (!IS_ERR(data->vcc_i2c)) {
		ret = regulator_set_voltage(data->vcc_i2c, 0,
			FT_I2C_VTG_MAX_UV);
		if (ret < 0)
			dev_err(&data->client->dev,
	...
	if (!IS_ERR(data->vdd)) {
		ret = regulator_set_voltage(data->vdd, 0, FT_VTG_MAX_UV);
		if (ret < 0)
			dev_err(&data->client->dev,
...
/**
 * ft5402_power_init - Initialize device power
 * @ts: driver private data
 *
 * Returns zero on success, else an error.
 */
static int ft5402_power_init(struct ft5402_ts_data *data)
{
	int ret;
...
/**
 * ft5402_power_deinit - Deinitialize device power
 * @ts: driver private data
 *
 * Returns zero on success, else an error.
 */
static int ft5402_power_deinit(struct ft5402_ts_data *data)
{
...
/*static int ft5402_power_on(struct ft5402_ts_data *data, bool on)
{
	int rc;
...
static int ft5402_power_init(struct ft5402_ts_data *data, bool on)
{
	int rc;
...
#ifdef CONFIG_PM
static int ft5402_ts_suspend(struct device *dev)
{
	struct ft5402_ts_data *data = dev_get_drvdata(dev);
	char txbuf[2], i;
	//int err;
	printk("%s\n",__func__);
	...
	if (gpio_is_valid(data->pdata->reset_gpio)) {
		...
		ft5402_i2c_write(data->client, txbuf, sizeof(txbuf));
	}

	data->suspended = true;
	gpio_set_value(data->pdata->reset_gpio, 0);
	//if (data->power_on) {
	//			ft5402_power_off(data);
	//			data->power_on=false;
	//		}
...
static int ft5402_ts_resume(struct device *dev)
{
	struct ft5402_ts_data *data = dev_get_drvdata(dev);
	//int err;
	...
	msleep(300);
		...
		ft5402FL_Init_IC_Param(data->client);
				 ...
				 unsigned long event, void *data)
{
	...
	int *blank;
	struct ft5402_ts_data *ft5402_data =
		container_of(self, struct ft5402_ts_data, fb_notif);

	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
			ft5402_data && ft5402_data->client) {
		blank = evdata->data;
		if (*blank == FB_BLANK_UNBLANK)
			ft5402_ts_resume(&ft5402_data->client->dev);
		else if (*blank == FB_BLANK_POWERDOWN)
			ft5402_ts_suspend(&ft5402_data->client->dev);
	}

	return 0;
}
#elif defined(CONFIG_HAS_EARLYSUSPEND)
static void ft5402_ts_early_suspend(struct early_suspend *handler)
{
	struct ft5402_ts_data *data = container_of(handler,
						   struct ft5402_ts_data,
						   early_suspend);

	ft5402_ts_suspend(&data->client->dev);
}

static void ft5402_ts_late_resume(struct early_suspend *handler)
{
	struct ft5402_ts_data *data = container_of(handler,
						   struct ft5402_ts_data,
						   early_suspend);

	ft5402_ts_resume(&data->client->dev);
}
#endif

static const struct dev_pm_ops ft5402_ts_pm_ops = {
#if (!defined(CONFIG_FB) && !defined(CONFIG_HAS_EARLYSUSPEND))
	.suspend = ft5402_ts_suspend,
	.resume = ft5402_ts_resume,
#endif
};
#endif

static int ft5402_auto_cal(struct i2c_client *client)
{
	struct ft5402_ts_data *data = i2c_get_clientdata(client);
	u8 temp = 0, i;
...
static int ft5402_fw_upgrade_start(struct i2c_client *client,
			const u8 *data, u32 data_len)
{
	struct ft5402_ts_data *ts_data = i2c_get_clientdata(client);
	...
	for (i = 0, j = 0; i < FT_UPGRADE_LOOP; i++) {
		...
		ft5402_i2c_write(client, w_buf, 1);
		...
		ft5402_i2c_read(client, w_buf, 4, r_buf, 2);
	...
	w_buf[0] = 0xcd;
	ft5402_i2c_read(client, w_buf, 1, r_buf, 1);
	...
	for (i = 0; i < pkt_num; i++) {
		...
		ft5402_i2c_write(client, pkt_buf,
	...
	if ((data_len) % FT_FW_PKT_LEN > 0) {
		...
		ft5402_i2c_write(client, pkt_buf, temp + FT_FW_PKT_META_LEN);
		msleep(FT_FW_PKT_DLY_MS);
	}
...
static int ft5402_fw_upgrade(struct device *dev, bool force)
{
	struct ft5402_ts_data *data = dev_get_drvdata(dev);
	...
	if (FT_FW_CHECK(fw)) {
		rc = ft5402_fw_upgrade_start(data->client, fw->data, fw->size);
		if (rc < 0)
			dev_err(dev, "update failed (%d). try later...\n", rc);
		else if (data->pdata->info.auto_cal)
			ft5402_auto_cal(data->client);
	} else {
		dev_err(dev, "FW format error\n");
		rc = -EIO;
	}

	ft5402_update_fw_ver(data);
...
static ssize_t ft5402_update_fw_show(struct device *dev,
...
static ssize_t ft5402_update_fw_store(struct device *dev,
...
static DEVICE_ATTR(update_fw, 0664, ft5402_update_fw_show,
				ft5402_update_fw_store);

static ssize_t ft5402_force_update_fw_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct ft5402_ts_data *data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(force_update_fw, 0664, ft5402_update_fw_show,
				ft5402_force_update_fw_store);

static ssize_t ft5402_fw_name_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct ft5402_ts_data *data = dev_get_drvdata(dev);
...
static ssize_t ft5402_fw_name_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct ft5402_ts_data *data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(fw_name, 0664, ft5402_fw_name_show, ft5402_fw_name_store);

static bool ft5402_debug_addr_is_valid(int addr)
{
	if (addr < 0 || addr > 0xFF) {
...
static int ft5402_debug_data_set(void *_data, u64 val)
{
	struct ft5402_ts_data *data = _data;

	mutex_lock(&data->input_dev->mutex);

	if (ft5402_debug_addr_is_valid(data->addr))
		dev_info(&data->client->dev,
...
static int ft5402_debug_data_get(void *_data, u64 *val)
{
	struct ft5402_ts_data *data = _data;
	int rc;
	u8 reg;
...
DEFINE_SIMPLE_ATTRIBUTE(debug_data_fops, ft5402_debug_data_get,
			ft5402_debug_data_set, "0x%02llX\n");

static int ft5402_debug_addr_set(void *_data, u64 val)
{
	struct ft5402_ts_data *data = _data;

	if (ft5402_debug_addr_is_valid(val)) {
		mutex_lock(&data->input_dev->mutex);
		data->addr = val;
		mutex_unlock(&data->input_dev->mutex);
	}
...
static int ft5402_debug_addr_get(void *_data, u64 *val)
{
	struct ft5402_ts_data *data = _data;
...
DEFINE_SIMPLE_ATTRIBUTE(debug_addr_fops, ft5402_debug_addr_get,
			ft5402_debug_addr_set, "0x%02llX\n");

static int ft5402_debug_suspend_set(void *_data, u64 val)
{
	struct ft5402_ts_data *data = _data;
...
static int ft5402_debug_suspend_get(void *_data, u64 *val)
{
	struct ft5402_ts_data *data = _data;
...
DEFINE_SIMPLE_ATTRIBUTE(debug_suspend_fops, ft5402_debug_suspend_get,
			ft5402_debug_suspend_set, "%lld\n");

static int ft5402_debug_dump_info(struct seq_file *m, void *v)
{
	struct ft5402_ts_data *data = m->private;
...
{
	return single_open(file, ft5402_debug_dump_info, inode->i_private);
}
...
#ifdef CONFIG_OF
static int ft5402_get_dt_coords(struct device *dev, char *name,
				struct ft5402_ts_platform_data *pdata)
{
...
static int ft5402_parse_dt(struct device *dev,
			struct ft5402_ts_platform_data *pdata)
{
	int rc;
	struct device_node *np = dev->of_node;
	...
	rc = ft5402_get_dt_coords(dev, "focaltech,panel-coords", pdata);
	if (rc && (rc != -EINVAL))
		return rc;

	rc = ft5402_get_dt_coords(dev, "focaltech,display-coords", pdata);
	if (rc)
		return rc;
...
static int ft5402_ts_probe(struct i2c_client *client,
			   const struct i2c_device_id *id)
{
	struct ft5402_ts_platform_data *pdata;
	struct ft5402_ts_data *data;
	struct input_dev *input_dev;
	struct dentry *temp;
	u8 reg_value;
	u8 reg_addr;
	...
	if (client->dev.of_node) {
		pdata = devm_kzalloc(&client->dev,
			sizeof(struct ft5402_ts_platform_data), GFP_KERNEL);
		if (!pdata) {
			dev_err(&client->dev, "Failed to allocate memory\n");
			return -ENOMEM;
		}

		err = ft5402_parse_dt(&client->dev, pdata);
		if (err) {
	...
	data = devm_kzalloc(&client->dev,
			sizeof(struct ft5402_ts_data), GFP_KERNEL);
	if (!data) {
...
printk("pdata->x_max = %d,pdata->y_max= %d\n",pdata->x_max,pdata->y_max);
	...
	err = ft5402_power_init(data);
		if (err) {
			dev_err(&client->dev, "GTP power init failed\n");
			goto exit_power_off;
		}
	
		err = ft5402_power_on(data);
		if (err) {
	...
	if (gpio_is_valid(pdata->irq_gpio)) {
		err = gpio_request(pdata->irq_gpio, "ft5402_irq_gpio");
		if (err) {
...
printk("pdata->irqflags = %d\n",pdata->irq_gpio_flags);


	err = request_threaded_irq(client->irq, NULL,
					   ft5402_ts_interrupt, pdata->irq_gpio_flags, 
	...
	if (gpio_is_valid(pdata->reset_gpio)) {
		err = gpio_request(pdata->reset_gpio, "ft5402_reset_gpio");
		if (err) {
	...
	msleep(300);
		...
		ft5402FL_Init_IC_Param(client);
	
	msleep(300);

	/* check the controller id */
	reg_addr = FT_REG_ID;
	err = ft5402_i2c_read(client, &reg_addr, 1, &reg_value, 1);
	if (err < 0) {
	...
	ft5402_i2c_read(client, &reg_addr, 1, &reg_value, 1);
	if (err < 0)
	...
	data->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN +
						    FT_SUSPEND_LEVEL;
	data->early_suspend.suspend = ft5402_ts_early_suspend;
	data->early_suspend.resume = ft5402_ts_late_resume;
...
static int  ft5402_ts_remove(struct i2c_client *client)
{
	struct ft5402_ts_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id ft5402_ts_id[] = {
	{"ft5402_ts", 0},
	{},
};

MODULE_DEVICE_TABLE(i2c, ft5402_ts_id);

#ifdef CONFIG_OF
static struct of_device_id ft5402_match_table[] = {
	{ .compatible = "focaltech,5402",},
	{ },
};
#else
#define ft5402_match_table NULL
#endif

static struct i2c_driver ft5402_ts_driver = {
	.probe = ft5402_ts_probe,
	.remove = ft5402_ts_remove,
	.driver = {
		   .name = "ft5402_ts",
		   .owner = THIS_MODULE,
		.of_match_table = ft5402_match_table,
#ifdef CONFIG_PM
		   .pm = &ft5402_ts_pm_ops,
#endif
		   },
	.id_table = ft5402_ts_id,
};

static int __init ft5402_ts_init(void)
{
	printk("%s\n",__func__);
	return i2c_add_driver(&ft5402_ts_driver);
}
module_init(ft5402_ts_init);

static void __exit ft5402_ts_exit(void)
{
	i2c_del_driver(&ft5402_ts_driver);
}
module_exit(ft5402_ts_exit);

MODULE_DESCRIPTION("FocalTech ft5402 TouchScreen driver");
