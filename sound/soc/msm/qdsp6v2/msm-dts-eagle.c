...
static s32 _get_cb_for_dev(int device)
{
	...
	pr_err("DTS_EAGLE_DRIVER: %s - device %i not found\n",
		   __func__, device);
	return -EINVAL;
}
...
static void _unreg_ion_mem(void)
{
	...
	if (rc)
		pr_err("DTS_EAGLE_DRIVER: %s - msm audio ion free failed\n",
			__func__);
}
...
static int _sendcache_post(int port_id, int copp_idx, int topology)
{
	...
	if (port_id == -1) {
		cidx = _get_cb_for_dev(_device_primary);
		if (cidx < 0) {
			pr_err("DTS_EAGLE_DRIVER_SENDCACHE_POST: %s, no cache for primary device %i found. Port id was 0x%X\n",
...
int msm_dts_eagle_set_stream_gain(struct audio_client *ac, int lgain, int rgain)
{
	...
	for (i = 0; i < _vol_cmd_cnt; i++) {
		...
		if (idx < 0) {
			pr_err("DTS_EAGLE_DRIVER_VOLUME: in %s, no cache for primary device %i found\n",
				__func__, _device_primary);
			return -EINVAL;
		}
		...
		if (depd->offset == -1) {
			...
			if (!buf_m) {
				...
				return -ENOMEM;
						       ...
						       depd->size, buf_m,
						       po, mod) < 0) {
				pr_err("DTS_EAGLE_DRIVER_ASM: asm get failed\n");
				ret = -EFAULT;
			...
			if (tgt < 0) {
				pr_err("DTS_EAGLE_DRIVER_ASM: in %s, no cache for device %i found\n",
			     ...
			     bool for_pre, bool get)
{
	...
	if (_isNTDevice(depd->device)) {
		...
		cidx = adm_validate_and_get_port_index(pid);
		pr_debug("DTS_EAGLE_DRIVER_ADM: adm_dts_eagle_set called with id = 0x%X, size = %i, offset = %i, device = %i, port id = %u, copp index = %u\n",
				depd->id, depd->size, depd->offset,
				depd->device, pid, cidx);
		ret = adm_dts_eagle_set(pid, _cidx[cidx], depd->id,
					(void *)buf, depd->size);
		if (ret < 0)
			pr_err("DTS_EAGLE_DRIVER_ADM: adm_dts_eagle_set failed\n");
		else
			pr_debug("DTS_EAGLE_DRIVER_ADM: adm_dts_eagle_set succeeded\n");
	} else {
		ret = -EINVAL;
...
int msm_dts_eagle_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case DTS_EAGLE_IOCTL_GET_PARAM: {
		...
		if (depd.offset == -1) {
			...
			if (cb < 0) {
				pr_err("DTS_EAGLE_DRIVER_IOCTL: in %s, no cache for device %i found\n",
					__func__, depd.device);
				return -EINVAL;
			}
