				...
				enum snd_soc_bias_level level)
{
	...
	if (dapm->dev != codec_dai->dev) {
		dev_err(codec->dev, "%s(%d) : No Codec DAI  dapm->dev(%s), codec_dai->dev(%s,%s)\n", 
	...
	case SND_SOC_BIAS_PREPARE:
		...
		dev_err(codec->dev, " Ignore bias levle (%d) On, Pre level (%d)\n", level, data->previous_bias_level);
		break;
        }
	/*
 ...
 */
 	dev_err(codec->dev, "  Set Sysclk : Dai name '%s', bias level (%d) On, fll_out(%d)\n",  codec_dai->name, level, data->fll_out);
				     ...
				     enum snd_soc_bias_level level)
{
	...
	if (dapm->dev != codec_dai->dev) {
		dev_err(codec->dev, "%s(%d) : No Codec DAI\n", __func__, __LINE__);
		return 0;
	}
	...
	case SND_SOC_BIAS_STANDBY:
		...
		dev_err(codec->dev, " Ignore bias levle (%d) Off\n", level);
            	break;
        }
	dev_err(codec->dev, "Close Sysclk: Dai name '%s', Bias level (%d)  Off, Pre level (%d)\n",codec_dai->name, level, data->previous_bias_level);
...
#undef S
	 ...
	 */
#if defined(CONFIG_L8720_COMMON)	// xuke @ 20141226 20150202 20150306
	btn_low[0] = 75;	// BTN_0
	btn_high[0] = 75;
	btn_low[1] = 100;	// BTN_1	
	btn_high[1] = 100;
